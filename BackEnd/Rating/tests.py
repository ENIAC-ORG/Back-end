...
class RatingViewSetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.rating_url = reverse("Rate")  # f"{settings.WEBSITE_URL}/Rating/Rate/"
    ...
    def test_rating_creation_success(self, mock_rating_filter):
        ...
        self.assertEqual(
            response.data["psychiatrist"], self.valid_payload["psychiatrist"]
        )
    ...
    def test_rating_creation_duplicate(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data["error"], "You have already rated this psychiatrist."
        )
